...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    // 获取对象中对应的键值
    // @parma obj 源数据对象
    // @parma str 要获取的数据对象中指定键的值 eg. 'data.list'
    Class.prototype.objData = function (obj, str) {
        str = str.split('.');
        var tmp = str.shift();
        while (tmp) {
            if (obj.hasOwnProperty(tmp)) obj = obj[tmp];
            else return null;
            tmp = str.shift();
        }
        return obj;
    ...
    // 设置对象中对应键的值
    // @parma obj 要修改的对象
    // @parma str 要设置的数据对象中对应的键名 eg. 'data.list'
    // @parma value 要设置的值
    Class.prototype.setObjData = function (obj, str, value) {
        var data = obj;
        str = str.split('.');
        var tmp = str.shift();
        while (tmp) {
    ...
    //获得数据
    Class.prototype.pullData = function (curr, loadIndex, apiSort) {
        var that = this,
            options = that.config,
        ...
        if (options.url) { //Ajax请求
            ...
            params['_ts'] = new Date().getTime(); // 兼容 IE 防止缓存

            //参数
            var search = options.search || {};
            var data = $.extend(params, options.where, search);
            options.search = {};
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res[response.statusName] != response.statusCode) {
                        ...
                        that.renderData(res, curr, that.objData(res, response.countName), apiSort), sort();
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        ...
        var that = this,
            options = that.config,
            data = that.objData(res, options.response.dataName) || [],
            trs = [],
            trs_fixed = [],
            trs_fixed_r = []
            ...
            options.page = $.extend({
                ...
                jump: function (obj, first) {
                    if (!first) {
                        ...
                        that.pullData(obj.curr, that.loading(), true);
                    }
                }
            }, options.page);
    ...
    Class.prototype.sort = function (th, type, pull, formEvent) {
        ...
        // CHANGE
        var sortOptions = options.apiSort;
        if (sortOptions) {
            // 后台排序
            var sortName = sortOptions.sortName || 'sortBy';
            var sortType = sortOptions.sortType || 'order';
            if (type === 'asc') { //升序
                options.where[sortName] = field;
                options.where[sortType] = sortOptions.asc || 'asc';
            } else if (type === 'desc') { //降序
                options.where[sortName] = field;
                options.where[sortType] = sortOptions.desc || 'desc';
            } else { //清除排序
                options.where = {};
                delete that.sortKey;
            }
            that.pullData(that.page, that.loading(), true);
        } else {
            // 前端排序
            if (type === 'asc') { //升序
            ...
            // CHANGE
            res = that.setObjData(res, options.response.dataName, thisData);
    ...
    Class.prototype.events = function () {
        ...
        that.layBody.on('click', 'tr', function (e) {
            ...
            // 阻止编辑框冒泡
            if (e.target.dataset.edit == 'text' || e.target.parentElement.dataset.edit == 'text') return;
            // 组织固定列点击事件
            if (e.currentTarget.parentNode.parentNode.parentNode.parentNode.className.indexOf('layui-table-fixed') != -1) return;
    ...
    // 搜索
    table.search = function (id, where) {
        var config = thisTable.config[id];
        where = where || {};
        if (!config) return hint.error('The ID option was not found in the table instance');
        config.search = where;
